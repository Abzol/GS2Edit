...
/* Load a file and scan it for file version */
EXPORTREAL loadFile(char* name)
{
    if (fstream!=NULL) { // close any open file handles
        fclose(fstream);
    }
    fstream=fopen(name, "r+b"); //open ROM in binary read mode
    // Game version checking
    // This sets filetable to hold the offset for the start of the data
    if (fstream!=NULL) {
        if (LOAD_INT(0x080000AB) == 0x53474141) {
            unsigned char l; //I dont know what this is for since its never used anywhere
            fileversion=0;
            switch (LOAD_BYTE(0x080000AF)){
                case 69: /* Fall through */
                case 73: filetable=0x8320000; break;
                case 74: filetable=0x8317000; break;
                case 68: filetable=0x831FE00; break;
                case 70: /* Fall through */
                case 83: filetable=0x8321800; break;
            }
...
EXPORTREAL replaceString(double ind, unsigned char* str)
{
    ...
    int ppos, pos, pos2, bpos, bpos2, length, length2, size, num, i, j, psize, psize2, index;
    index  = int(ind);
    bpos   = 0;
	bpos2  = 0;
	psize  = 0;
...
EXPORTREAL decompressMap(double pos, double format, char* outfile)
{
    ...
    if (fstream_o == NULL) {
        ...
        for (pos1 = 0x8000; pos1 < 0x10000; pos1 += 2) {
            ...
            buffer[pos2] = num & 255;
			buffer[pos2 + 1] = (num >> 8) & 255;
			pos2 += 4;
        }
        pos2 = 0x4000;
        pos3 = 2;
        for (pos1 = 0; pos1 < 0x4000; ++pos1) {
            buffer[pos3] = bufferI[pos1];
			buffer[pos3 + 1] = bufferI[pos2];
			++pos2;
			pos3 += 4;
        }
        length = fwrite(buffer, 1, length, fstream_o);
...
EXPORTREAL decompress16(double pos, char* outfile, double size, double pal)
{
    ...
    if (fstream_o == NULL) {
        ...
        buffer[0]  = 66;
		buffer[1]  = 77;
        buffer[2]  =  (length + 74) & 255;
		buffer[3]  = ((length + 74) >>  8) & 255;
		buffer[4]  = ((length + 74) >> 16) & 255;
		buffer[5]  = ((length + 74) >> 24) & 255;
        buffer[6]  = 0;
		buffer[7]  = 0;
        buffer[8]  = 0;
		buffer[9]  = 0;
        ...
        for (i = 1; i <= hig; ++i) {
            dy = (hig - i) * wid;
            for (j = 0; j < (wid >> 1); ++j) {
                buffer[74 + j + i * (wid >> 1)] = (bufferI[(j << 1) + 1 + dy] & 15) | ((bufferI[(j << 1) + dy] & 15) << 4);
            }
        }
...
EXPORTREAL createTileset(char* tilefile, char* palfile, char* outfile)
{
	...
	buffer[1]  = 77;
    buffer[2]  = (length) & 255;
	buffer[3]  = (length >>  8) & 255;
	buffer[4]  = (length >> 16) & 255;
	buffer[5]  = (length >> 24) & 255;
    buffer[6]  = 0;
	buffer[7]  = 0;
    buffer[8]  = 0;
	buffer[9]  = 0;
...
/* Rotate a rightwards b. Some people will know this as rcr */
